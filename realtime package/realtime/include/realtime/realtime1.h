...
void segment(pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud,std_msgs::Int32MultiArray dat,ros::Publisher pub)
{
  ...
  ec.setClusterTolerance (0.18); // 2cm
...
void callback(sensor_msgs::PointCloud2ConstPtr const& pcloud2,std_msgs::Int32MultiArray dat,ros::Publisher pub)
{
    printf("--------------------------------------------\n");
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr outputCloud = temp.makeShared();
    pcl::PointCloud<pcl::PointXYZ> temp1;
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_filtered = temp.makeShared();
    ...
    pcl::removeNaNFromPointCloud(*tcloud2, *outputCloud, indices);
    std::cerr << "Cloud before filtering: " << std::endl;
//   for (size_t i = 0; i < outputCloud->points.size (); ++i){
//     std::cerr << "    " << outputCloud->points[i].x << " " 
//                         << outputCloud->points[i].y << " " 
//                         << outputCloud->points[i].z << std::endl;
// }
    pcl::PassThrough<pcl::PointXYZ> pass;
  pass.setInputCloud (outputCloud);
  ...
  pass.filter (*outputCloud);
    ...
    // // segment("cloud1.ply");
    // pcl::PCDWriter writer;
    // writer.write<pcl::PointXYZ> ("cloud.pcd", *outputCloud, false);
    segment(outputCloud,dat,pub);
    // segment("cloud3.ply");
    // // segment("cloud4.ply");
