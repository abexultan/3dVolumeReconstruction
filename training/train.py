...
###############################################################
config={}
config['train_names'] = ['akmaral', 'albina', 'alen', 'bex', 'beka', 'mika', 'nazerke', 'shamil', 'lashin', 'denis']
for name in config['train_names']:
	for d in range(1,4):
		for s in range(1,11):
   	 		config['X_train_'+name] = '/raid/GradProject/Data/'+name+'/day'+str(d)+'/'+name+'_scenario'+str(s)+'_clouds64_25d/voxel_grids_64/'
    			config['Y_train_'+name] = '/raid/GradProject/Data/'+name+'/day'+str(d)+'/'+name+'_scenario'+str(s)+'_clouds64_3d/voxel_grids_64/'
config['test_names']=['lera', 'alfarabi', 'askhat', 'zhanel', 'zhanat']
for name in config['test_names']:
	for d in range(1,4):
		for s in range(1,11):
    			config['X_test_'+name] = '/raid/GradProject/Data/'+name+'/day'+str(d)+'/'+name+'_scenario'+str(s)+'_clouds64_25d/voxel_grids_64/'
    			config['Y_test_'+name] = '/raid/GradProject/Data/'+name+'/day'+str(d)+'/'+name+'_scenario'+str(s)+'_clouds64_3d/voxel_grids_64/'
...
class Network:
    ...
    def ae_u(self,X):
        with tf.device('/gpu:'+GPU0):
            X = tf.reshape(X,[-1,resolution,resolution,resolution,1])
            ##### encode
    ...
    def dis(self, X, Y):
        with tf.device('/gpu:'+GPU0):
            X = tf.reshape(X,[-1,resolution,resolution,resolution,1])
            Y = tf.reshape(Y,[-1,resolution,resolution,resolution,1])
	    ...
	    d1 = int(d1); d2 = int(d2); d3 = int(d3); cc = int(cc)
            y = tf.reshape(layers_d[-1],[-1, int(d1)*int(d2)*int(d3)*int(cc)])
        return tf.nn.sigmoid(y)

    def train(self, data):
        X = tf.placeholder(shape=[None, resolution, resolution, resolution, 1], dtype=tf.float32)
        Y = tf.placeholder(shape=[None, resolution, resolution, resolution, 1], dtype=tf.float32)
        ...
        with tf.device('/gpu:'+GPU0):
            ################################ ae loss
            Y_ = tf.reshape(Y,shape=[-1, resolution**3])
            Y_pred_modi_ = tf.reshape(Y_pred_modi,shape=[-1, resolution**3])
            w = 0.85
            ...
            Y_pred_ = tf.reshape(Y_pred,shape=[-1, resolution**3])
        ...
        with tf.Session(config=config) as sess:
            ...
            for epoch in range(15):
                ...
                for i in range(total_train_batch_num):
                    ...
                    #### model saving
                    if i %250==0 and i>0 and epoch % 1 == 0:
                        saver.save(sess, save_path=self.train_mod_dir +'model.cptk')
			tf.train.write_graph(sess.graph.as_graph_def(), self.train_mod_dir, 'tensorflowModel.pbtxt', as_text=True)
